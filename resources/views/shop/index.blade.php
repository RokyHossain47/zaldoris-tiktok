...
<head>
    ...
    <meta name="description" content="{{ setting('meta_description', 'Zaldoris - Live Shopping Feed. Experience real-time live video shopping, featured streams, and exclusive deals.') }}">
    <title>Live Shopping Feed - {{ setting('site_name', 'Zaldoris Live Commerce Platform') }}</title>
    <!-- Favicon -->
...
<main class="page-container">
    ...
    <section class="section-spacing">
        <div class="section-header-row mb-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('shop.index', ['filter' => 'featured']) }}" class="btn-outline-follow {{ request('filter') === 'featured' ? 'active' : '' }}" style="text-decoration:none; padding: 4px 12px; font-size: 13px;">Featured Products</a>
                <a href="{{ route('shop.index', ['filter' => 'trending']) }}" class="btn-outline-follow {{ request('filter') === 'trending' ? 'active' : '' }}" style="text-decoration:none; padding: 4px 12px; font-size: 13px;">Trending Products</a>
            </div>
        </div>
        <div class="top-categories-row" style="overflow-x: auto; padding-bottom: 8px;">
            <a href="{{ route('shop.index') }}" class="category-pill-btn {{ !request('category') || request('category') === 'all' ? 'active' : '' }}" style="text-decoration:none; display: inline-flex; align-items:center; gap:6px;">
                <i class="bi bi-grid-fill"></i> All Categories
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('shop.index', ['category' => $cat->slug]) }}" class="category-pill-btn {{ request('category') === $cat->slug ? 'active' : '' }}" style="text-decoration:none; display: inline-flex; align-items:center; gap:6px;">
                @if($cat->icon)
                    <i class="{{ $cat->icon }}"></i>
                @else
    ...
    <section class="section-spacing">
        <div class="section-header-row">
            ...
            <span class="text-muted" style="font-size: 13px;">Showing {{ $products->count() }} of {{ $products->total() }} items</span>
        </div>
        ...
        <div class="grid-5-col">
            ...
            @endphp
            <a href="{{ route('shop.product', $p->id) }}" class="zal-card" style="text-decoration:none;">
                <div class="live-card-thumb" style="position:relative;">
                    <img src="{{ $img }}" alt="{{ $p->title }}" style="width:100%; height:180px; object-fit:cover; border-radius:12px;">
                    @if($p->is_featured)
                    ...
                    <div class="badge-viewers-top" style="background:rgba(9,13,16,0.85); color:#00F0C8; font-weight:700;">
                        {{ setting('currency_symbol', '$') }}{{ number_format($p->price, 2) }}
                    </div>
                </div>
                <div class="live-card-body" style="padding:10px 0;">
                    ...
                    <div class="host-row" style="margin-top:6px; display:flex; align-items:center; justify-content:space-between;">
                        <span style="font-size:12px; color:#6B7C93;">
                            {{ $p->category->name ?? $p->category ?? 'Product' }}
                        </span>
    ...
    <section class="section-spacing">
        <div class="section-header-row">
            ...
            <a href="{{ route('streams.index') }}" class="view-all-link">View All <i class="bi bi-arrow-right"></i></a>
        </div>

        <div class="grid-5-col">
            <!-- Card 1 -->
            <a href="{{ route('streams.show', 1) }}" class="zal-card">
