...
<div style="max-width: 960px; margin: 0 auto;">
    ...
    <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 32px; box-shadow: 0 10px 30px rgba(0,0,0,0.4);">
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="productCreateForm">
            ...
            <div style="background: var(--bg-card-inner); border: 2px solid rgba(0, 240, 200, 0.3); border-radius: 14px; padding: 20px; margin-bottom: 28px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <label class="product-type-option" style="background: rgba(255, 255, 255, 0.03); border: 1px solid var(--border-color); border-radius: 12px; padding: 16px; cursor: pointer; display: flex; gap: 12px; align-items: flex-start; transition: all 0.2s ease;">
                        <input type="radio" name="is_live_product" value="0" {{ ($defaultType ?? 'normal') === 'normal' ? 'checked' : '' }} onchange="toggleLiveStreamFields(false)" style="margin-top: 4px; accent-color: var(--cyan-accent);">
                        <div>
                    ...
                    <label class="product-type-option" style="background: rgba(255, 255, 255, 0.03); border: 1px solid var(--border-color); border-radius: 12px; padding: 16px; cursor: pointer; display: flex; gap: 12px; align-items: flex-start; transition: all 0.2s ease;">
                        <input type="radio" name="is_live_product" value="1" {{ ($defaultType ?? 'normal') === 'live' ? 'checked' : '' }} onchange="toggleLiveStreamFields(true)" style="margin-top: 4px; accent-color: var(--pink-accent);">
                        <div>
            ...
            <div id="liveStreamRoomBox" style="display: {{ ($defaultType ?? 'normal') === 'live' ? 'block' : 'none' }}; background: rgba(254, 44, 85, 0.08); border: 1px solid rgba(254, 44, 85, 0.3); border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <div>
                    ...
                    <select name="stream_id" style="width: 100%; background: var(--bg-card-inner); border: 1px solid var(--border-color); color: #fff; padding: 12px 16px; border-radius: 10px; font-size: 14px;">
                        ...
                        @foreach($streams as $st)
                            <option value="{{ $st->id }}">{{ $st->title }} (Host: {{ $st->host ? $st->host->name : 'Admin' }} &bull; {{ $st->viewer_count }} viewers)</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 24px;">
                ...
                <div>
                    <label style="display: block; font-size: 14px; font-weight: 700; color: #fff; margin-bottom: 8px;">Compare At / Strike Price ({{ setting('currency_symbol', 'C$') }})</label>
