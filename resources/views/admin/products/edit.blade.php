...
@section('title', 'Edit Product - Super Admin')
...
<div style="max-width: 900px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            ...
            <p style="font-size: 14px; color: var(--text-muted);">Update product details, pricing, inventory, and promotional badges.</p>
        </div>
    ...
    <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 32px; box-shadow: 0 10px 30px rgba(0,0,0,0.4);">
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <!-- IMAGE UPLOAD -->
            <div style="margin-bottom: 24px;">
                <label style="display: block; font-size: 14px; font-weight: 700; color: #fff; margin-bottom: 8px;">Product Image Preview & Replace</label>
                <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 12px;">
                    <img src="{{ $product->primary_image }}" alt="Current Image" style="width: 70px; height: 70px; object-fit: cover; border-radius: 10px; border: 1px solid var(--border-color);">
                    <div style="flex: 1;">
                ...
                <div style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">Or comma-separated Image URLs:</div>
                <input type="text" name="images" value="{{ is_array($product->images) ? implode(', ', $product->images) : $product->images }}" style="width: 100%; background: var(--bg-card-inner); border: 1px solid var(--border-color); color: #fff; padding: 12px 16px; border-radius: 10px; font-size: 14px; margin-top: 6px;">
            </div>
            ...
            <div style="margin-bottom: 28px;">
                ...
                <select name="status" style="width: 100%; background: var(--bg-card-inner); border: 1px solid var(--border-color); color: #fff; padding: 14px 16px; border-radius: 10px; font-size: 14px;">
                    <option value="active" {{ $product->status === 'active' ? 'selected' : '' }}>Active & Available on Marketplace</option>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 14px;">
                ...
                <button type="submit" style="background: linear-gradient(135deg, var(--cyan-accent), #1ed6d0); color: #13141f; border: none; padding: 14px 32px; border-radius: 10px; font-weight: 800; font-size: 14px; cursor: pointer; box-shadow: 0 4px 14px rgba(37, 244, 238, 0.4);">
                    Save Changes
                </button>
            </div>
        </form>
