...
@section('title', 'Categories Management - Super Admin')
                ...
                <tr style="border-bottom: 1px solid var(--border-color); color: #fff;">
                    <td style="padding: 16px 20px;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            @if($cat->image)
                                <img src="{{ $cat->image }}" alt="{{ $cat->name }}" style="width: 44px; height: 44px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border-color);">
                            @endif
                    ...
                    <td style="padding: 16px 20px;">
                        <div style="font-weight: 700; color: #fff;">{{ $cat->name }}</div>
                        @if($cat->description)
                            <div style="font-size: 12px; color: var(--text-muted);">{{ Str::limit($cat->description, 40) }}</div>
                        @endif
                    </td>
                    ...
                    <td style="padding: 16px 20px;">
                        <span class="status-pill {{ $cat->is_active ? 'status-active' : 'status-blocked' }}">
                            {{ $cat->is_active ? 'Active' : 'Disabled' }}
                        </span>
                    </td>
                    <td style="padding: 16px 20px; text-align: right;">
                        <div style="display: flex; gap: 8px; justify-content: flex-end;">
                            <button onclick="openEditCategoryModal({{ json_encode($cat) }})" class="action-btn" title="Edit Category">
                                <i class="bi bi-pencil-fill" style="color: var(--cyan-accent);"></i>
                            </button>
                            <form action="{{ route('admin.categories.delete', $cat->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');" style="margin: 0;">
                                @csrf
                                @method('DELETE')
